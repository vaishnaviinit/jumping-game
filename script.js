...
class Player {
    ...
    update(deltaTime) {
        ...
        // Handle input with smoother movement
        if (keys['ArrowLeft'] || keys['a'] || keys['A'] || mobileInput.left) {
            ...
            this.velocityX *= 0.9; // Smoother friction
        }
    ...
    jump() {
        if (this.jumpCooldown <= 0) {
            ...
            this.onGround = false;
            this.jumpCooldown = 200; // Prevent multiple jumps
        }
    }
...
class Platform {
    ...
    draw() {
        // Platform with gradient and rounded edges
...
function generatePlatforms() {
    // Add starting platform (bigger and centered)
    platforms.push(new Platform(canvas.width / 2 - 70, canvas.height - 25, 140, 15));
    
    // Generate platforms going upward with better spacing
    for (let i = 1; i < 50; i++) {
        // Ensure platforms are reachable by limiting horizontal distance
        let x;
        if (i === 1) {
            // First platform should be close to starting position
            x = Math.random() * 200 + 100;
        } else {
            // Subsequent platforms within reasonable reach
            const prevPlatform = platforms[platforms.length - 1];
            const maxDistance = 150; // Maximum horizontal distance
        ...
        // Add bombs less frequently and not on early platforms
...
function generateMorePlatforms() {
    ...
    for (let y = highestPlatform - GAME_CONFIG.platformSpacing; y > camera.y - canvas.height * 2; y -= GAME_CONFIG.platformSpacing) {
        // Better platform positioning logic
    ...
    // Remove distant platforms and bombs
    ...
    for (let platform of platforms) {
        ...
        if (checkAABBCollision(playerBounds, platformBounds)) {
            ...
            if (player.velocityY > 0 && 
                ...
                // Jump with reduced force when landing
